...
// Покупка плагина (заглушка вместо открытия Telegram/Discord)
function buyPlugin(pluginName) {
    alert(`Запрос на покупку плагина "${pluginName}" пока недоступен. Контакты появятся позже.`);
}
...
function loadChannels() {
    ...
    channelAvatarWrapper.href = '#!';
    channelAvatarWrapper.title = `Канал будет доступен позже`;
    ...
    subscribeBtn.innerHTML = `Канал будет доступен позже`;
